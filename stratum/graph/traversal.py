...
def find_uncontrolled_paths(graph: RiskGraph) -> list[RiskPath]:
    ...
    # Step 6: Sort by severity then hops
...
def _analyze_path(
    ...
    if not has_hitl and sensitivity in ("pii", "financial", "credentials"):
        missing.append("hitl")

    # Severity computation
    if sensitivity in ("credentials", "pii") and dest_trust == "external" and not has_output_filter:
        severity = "CRITICAL"
    elif sensitivity == "financial" and dest_trust == "external" and not has_hitl:
        severity = "CRITICAL"
    elif sensitivity in ("pii", "financial") and not has_output_filter:
        severity = "HIGH"
